...
const socket = io("https://quizapp-2-ui7y.onrender.com");
    ...
    useEffect(() => {
        async function fetchLeaderboard() {
            try {
                ...
                console.log("📊 Leaderboard Fetched:", data.data.leaderboard);
            } catch (error) {
        ...
        // 🔴 Listen for live leaderboard updates from the server
        socket.on("leaderboardUpdate", (updatedLeaderboard) => {
            console.log("📢 Live Leaderboard Update:", updatedLeaderboard);
                    ...
                    <tbody>
                        {leaderboard.length > 0 ? (
                            leaderboard.map((entry, index) => (
                                <tr
                                    key={entry.userId}
                                    className={`text-lg ${index % 2 === 0 ? "bg-gray-100" : "bg-white"} hover:bg-gray-200 transition-all duration-200`}
                                >
                                    {/* Rank with icons */}
